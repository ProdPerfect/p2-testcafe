...
function streamHtml(url, cache, options)
{
	...
	var request = new Promise((resolve, reject) => {
		...
		var stream = got(url,  // TODO :: https://github.com/joepie91/node-bhttp/issues/3
			{
		...
		stream.on('redirect', response => redirects.push(response));

		stream.on('response', (orgResponse) => {
			orgResponse.redirects = redirects;
